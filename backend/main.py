...
async def lifespan(app: FastAPI):
    ...
    await seed_admin()          # ensure default admin exists
    yield
    await close_db()

app = FastAPI(title="DataLens AI — Student Analytics", version="2.0.0", lifespan=lifespan)
...
app.add_middleware(
    ...
    allow_origins=[
        "http://localhost:5173",
        "http://127.0.0.1:5173",
        os.getenv("FRONTEND_URL", ""),          # e.g. https://analytidata.vercel.app
    ],
...
@app.get("/")
async def root():
    return {"message": "DataLens AI Student Analytics API v2"}
